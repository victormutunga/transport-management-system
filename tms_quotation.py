...
For next option you only have to type Special Python Code:
 - Special
		...
		# 'control_route': fields.boolean('Control Factors'),
	...
	def _get_total(self, cr, uid, ids, field_name, arg, context=None):
		...
		for factor in self.browse(cr, uid, ids, context=context):
			...
			if factor.factor_type == 'distance': 
				...
				x = 1
			result += ((factor.fixed_amount if (factor.mixed or factor.factor_type=='travel') else 0.0)+ (factor.rate * x ))
			res[factor.id] = result # Pendiente generar cálculo especial
		return res
...
class tms_quotation_agreement_wizard(osv.osv_memory):
	...
	_columns = {
		...
		'upload_point': fields.char('Upload Point', size=128, readonly=False, required=False),
		'download_point': fields.char('Download Point', size=128, required=False ),
		'date_start': openerp.osv.fields.date('Date Star', required=False),
		'date_end': openerp.osv.fields.date('Date End', required=False),
	...
	def button_generate_agreement(self, cr, uid, ids, context=None):
		...
		if not product_id:
			raise osv.except_osv(
				...
				_('No se tiene Configurado un producto Flete con las caracteristicas (tms_category, =, freight),(tms_default_freight,=,True),(active,=, 1)'))
			...
			for quotation in tms_quotation_obj.browse(cr, uid, [active_id], context=context):
					...
					if route.mark:
						if route.route_id.expense_driver_factor:
							...
							if not product_id:
								raise osv.except_osv(
									_('Error, No se puede generar el Acuerdo!'),
									_('No se tiene Configurado un producto con las caracteristicas Flete (tms_category, =, freight),(tms_default_freight,=,True),(active,=, 1)'))
							product_browse = product_obj.browse(cr, uid, product_id, context=context)[0]
							
							product_salary_id = product_obj.search(cr, uid, [('tms_category', '=', 'salary'),('tms_default_salary','=',True),('active','=', 1)], limit=1)
							if not product_salary_id:
								raise osv.except_osv(
	                                _('Error al Crear el Acuerdo !'),
	                                _('No se tiene un producto configurado de tipo Salario y Default por Defecto'))
							product_br = product_obj.browse(cr, uid, product_salary_id, context)[0]
							xline_factor_salary = (0,0,{
									    		'name'          : product_br.name,
										        'category'      : 'driver',
										        'factor_type'   : 'travel',
										        'framework'     : 'Any',
										        'range_start'   : 0.0,
										        'range_end'     : 0.0,
										        'factor'        : 0.0,
										        'fixed_amount'  : quotation.salary_travel_days,
										        'mixed'         : False,
										        'factor_special_id': False,
										        'variable_amount' : False,
										        'total_amount'  : quotation.salary_travel_days,
										        'sequence'      : 0,
										        'notes'         : "Factor cargado de la Cotizacion "+ quotation.sequence,
										        'control'       : False,### Revisar si es necesario eliminarlo
										        'driver_helper' : False,

									})
							
							factor_driver_list.append(xline_factor_salary)
				...
				if quotation.agreement_id:
					...
					for factor in quotation.route_ids:
						if factor.mark:
							...
							xline_factor = (0,0,{
										        ...
										        'range_start'   : 1,
										        'range_end'     : x,
										        'factor'        : factor.rate if factor.rate else 0.0,
										        ...
										        'total_amount'  : factor.total_amount-factor.insurance,
										        'sequence'      : 0,
					...
					if quotation.total_security > 0.0:
						product_income_id = product_obj.search(cr, uid, [('tms_category', '=', 'insurance'),('tms_category_default','=',1),('active','=', 1)], limit=1)
						product_browse_income = product_obj.browse(cr, uid, product_income_id, context)
						fpos = quotation.partner_id.property_account_position.id or False
						if not product_income_id:
							raise osv.except_osv(
                                _('Error al Crear el Acuerdo !'),
                                _('No se tiene un producto configurado como Seguro por Defecto'))
						xline_insurance = (0,0,{
									'line_type': 'product',
									'name': product_browse_income[0].name,
									'sequence': 1,
									'product_id': product_income_id[0],
									'price_unit': quotation.total_security,
									#'tax_id' : [(6, 0, [_w for _w in fpos_obj.map_tax(cr, uid, fpos, product_browse_income[0].taxes_id)])],
									'product_uom_qty': 1,
									'product_uom': product_browse_income[0].uom_id.id,
									'control': False,
									})
						agreement_line_list.append(xline_insurance)
						#agreement_line_insurance_list.append(xline_insurance)
     ...
     #                            _('No se tiene un producto configurado de tipo Llantas o Tires'))
					...
					if quotation.diesel_travel_days > 0.0:
						product_income_id = product_obj.search(cr, uid, [('tms_category', '=', 'fuel'),('tms_category_default','=',1),('active','=', 1)], limit=1)
						product_browse = product_obj.browse(cr, uid, product_income_id, context)
						fpos = quotation.partner_id.property_account_position.id or False
						if not product_income_id:
							raise osv.except_osv(
                                _('Error al Crear el Acuerdo !'),
                                _('No se tiene un producto configurado de tipo Combustible por Defecto'))
						else:
							sq += 1
							xline_expense = (0,0,{
									'automatic_advance': False,
									...
									'name': product_browse[0].name,
									'sequence': sq,
									'price_unit': quotation.diesel_cost,
									'product_uom_qty': quotation.liters_diesel,
									'discount': 0.00,
					...
					if quotation.toll_travel_days > 0.0:
						product_income_id = product_obj.search(cr, uid, [('tms_category', '=', 'highway_tolls'),('tms_category_default','=',1),('active','=', 1)], limit=1)
						product_browse = product_obj.browse(cr, uid, product_income_id, context)
						fpos = quotation.partner_id.property_account_position.id or False
						if not product_income_id:
							raise osv.except_osv(
     ...
     #                            _('No se tiene un producto configurado de tipo Gastos Administrativos'))
					...
					if not route_mark_id:
							raise osv.except_osv(
                                _('Error !'),
                                _('No se tiene una Ruta Marcada para el Calculo'))
					vals = {
							...
							'agreement_line': [x for x in agreement_line_list] if agreement_line_list else [],
							...
							'payment_term': wizard.payment_term.id,
