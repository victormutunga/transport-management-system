odoo.define('report_general_travel.ReportWidget', function (require) {
'use strict';
...
var ReportWidget = Widget.extend({
    ...
    start: function() {
        QWeb.add_template("/report_general_travel/static/src/xml/tms_report_travel_line.xml");
    ...
    unfold: function(e) {
        ...
        return this.context_model.call('add_line', [[parseInt(context_id, 10)], parseInt(active_id, 10)]).then(function () { // First add the line to the model
            ...
            if (!isLoaded) { // If the lines have not yet been loaded
                ...
                new Model('tms.report.context.common').call('get_full_report_name_by_report_name', [report_name]).then(function (result) {
                    ...
                    var f = function (lines) {// After loading the line
                        self.context_model.call('get_columns_types', [[parseInt(context_id, 10)]]).then(function (types) {
                            ...
                            for (line in lines) { // Render each line
                                $cursor.after(QWeb.render("report_tms_travel_line", {l: lines[line], types: types}));
                                $cursor = $cursor.next();
                            }
                        });
                    };
